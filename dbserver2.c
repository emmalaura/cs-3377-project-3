...
main(int argc, char **argv) {
  ...
  while (1) {
    ...
    pthread_t tid;
    pthread_create(&tid, NULL, HandleClient, (void *)(intptr_t)c_fd);
    pthread_detach(tid);
  }
...
void *HandleClient(void *arg) {
  ...
  while (1) {
    ...
    ssize_t total_typed = 0;
    ssize_t remaining = strlen("You typed: ") + strlen(clientbuf);
    char *input = "You typed: ";

  while(total_typed < remaining){
    ssize_t typed = write(c_fd, input, remaining - total_typed);
    if(typed == -1){
